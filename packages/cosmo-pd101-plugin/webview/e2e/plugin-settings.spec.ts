...
test.describe("Synth settings", () => {
	test("exposes tempo and voice allocation from simple mode", async ({ page }) => {
		...
		await expect(settings.getByText("Performance", { exact: true })).toBeVisible();
		await expect(settings.getByText("Transport", { exact: true })).toBeVisible();
		await expect(
