//
//  CosmoPD101AUv3Ext_macOSExtensionDSPKernel.hpp
...
/*
 CosmoPD101AUv3Ext_macOSExtensionDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class CosmoPD101AUv3Ext_macOSExtensionDSPKernel {
public:
    ...
    // Add a case for each parameter in CosmoPD101AUv3Ext_macOSExtensionParameterAddresses.h
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case CosmoPD101AUv3Ext_macOSExtensionParameterAddress::gain:
                mGain = value;
                break;
        }
    }
    ...
    void noteOn(std::int32_t note, double frequency, double velocity) {
        mLastNote = note;
        mSinOsc = SinOscillator(mSampleRate);
        const auto targetFrequency = frequency > 0.0 ? frequency : MIDINoteToFrequency(note);
        mSinOsc.setFrequency(targetFrequency);
        mNoteEnvelope = std::clamp(velocity, 0.0, 1.0);
    }

    void noteOff(std::int32_t note) {
        if (mSustain && note == mLastNote) {
            return;
        }
        mNoteEnvelope = 0.0;
    }
    ...
    void handleMIDIEventList(AUEventSampleTime now, AUMIDIEventList const* midiEvent) {
        auto visitor = [] (void* context, MIDITimeStamp timeStamp, MIDIUniversalMessage message) {
            auto thisObject = static_cast<CosmoPD101AUv3Ext_macOSExtensionDSPKernel *>(context);
