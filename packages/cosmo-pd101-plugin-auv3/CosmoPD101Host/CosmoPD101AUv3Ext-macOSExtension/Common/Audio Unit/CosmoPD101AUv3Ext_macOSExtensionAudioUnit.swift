...
class TruceAUAudioUnit: AUAudioUnit, @unchecked Sendable {
    ...
    private var factoryPresetIds: [String] = []
    ...
    override init(componentDescription: AudioComponentDescription,
        ...
        if let container = FileManager.default.containerURL(
            forSecurityApplicationGroupIdentifier: "group.ca.purraudio.CosmoPD101Host"
        ) {
    ...
    override var midiOutputNames: [String] {
        if let d = g_descriptor?.pointee, d.has_midi_output != 0 || d.num_outputs == 0 {
            return ["MIDI Out"]
        }
        return []
    }
    ...
    override var factoryPresets: [AUAudioUnitPreset]? {
        guard let library = editorResult(method: "getPresetLibrary", args: [["source": "factory"]]) as? [String: Any],
              let entries = library["entries"] as? [[String: Any]] else {
            return []
        }
        factoryPresetIds = entries.compactMap { $0["id"] as? String }
        return entries.enumerated().compactMap { index, entry -> AUAudioUnitPreset? in
    ...
    override var currentPreset: AUAudioUnitPreset? {
        ...
        set {
            guard let preset = newValue, preset.number >= 0 else {
                selectedFactoryPreset = nil
                super.currentPreset = newValue
                return
            }
            if factoryPresetIds.isEmpty {
                _ = factoryPresets
            }
            let index = preset.number
            guard factoryPresetIds.indices.contains(index),
                  editorResult(method: "loadPresetData", args: [["id": factoryPresetIds[index]]]) != nil else {
                return
            }
            selectedFactoryPreset = preset
            super.currentPreset = preset
            if let ctx = rustCtx, let callbacks = g_callbacks, let tree = _parameterTree {
                for parameter in tree.allParameters {
                    parameter.value = AUValue(callbacks.pointee.param_get_value(ctx, UInt32(parameter.address)))
                }
            }
        }
    }

    private func editorResult(method: String, args: [Any] = []) -> Any? {
        guard let ctx = rustCtx, let callbacks = g_callbacks else { return nil }
        let request: [String: Any] = ["id": -1, "method": method, "args": args]
        ...
        guard let response, responseLength > 0 else { return nil }
        defer { callbacks.pointee.custom_editor_response_free(response, responseLength) }
        let data = Data(bytes: response, count: Int(responseLength))
    ...
    override var latency: TimeInterval {
        guard let ctx = rustCtx, let cb = g_callbacks, _sampleRate > 0 else { return 0 }
        return TimeInterval(cb.pointee.latency(ctx)) / _sampleRate
    }
    override var tailTime: TimeInterval {
        guard let ctx = rustCtx, let cb = g_callbacks, _sampleRate > 0 else { return 0 }
        return TimeInterval(cb.pointee.tail(ctx)) / _sampleRate
    }
