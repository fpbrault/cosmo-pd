...
impl CosmoProcessor {
    ...
    pub fn update_fx(&mut self) {
        let p = &self.params;
           // Classic effects
           self.fx.chorus.enabled = p.chorus.enabled;
           self.fx.chorus.rate = p.chorus.rate;
           self.fx.chorus.depth = p.chorus.depth;
           self.fx.chorus.mix = p.chorus.mix;

           self.fx.phaser.enabled = p.phaser.enabled;
           self.fx.phaser.rate = p.phaser.rate;
           self.fx.phaser.depth = p.phaser.depth;
           self.fx.phaser.mix = p.phaser.mix;
           self.fx.phaser.feedback = p.phaser.feedback;

           self.fx.delay.enabled = p.delay.enabled;
           self.fx.delay.time = p.delay.time;
           self.fx.delay.feedback = p.delay.feedback;
           self.fx.delay.mix = p.delay.mix;
           self.fx.delay.tape_mode = p.delay.tape_mode;
           self.fx.delay.warmth = p.delay.warmth;
           ...
           // New effects
           self.fx.compressor.enabled = p.compressor.enabled;
           self.fx.compressor.threshold_db = p.compressor.threshold_db;
           self.fx.compressor.ratio = p.compressor.ratio;
           self.fx.compressor.attack_ms = p.compressor.attack_ms;
           self.fx.compressor.release_ms = p.compressor.release_ms;
           self.fx.compressor.makeup_db = p.compressor.makeup_db;
           self.fx.compressor.mix = p.compressor.mix;

           self.fx.eq.enabled = p.eq.enabled;
           self.fx.eq.gains[0] = p.eq.gain80;
           self.fx.eq.gains[1] = p.eq.gain240;
           self.fx.eq.gains[2] = p.eq.gain750;
           self.fx.eq.gains[3] = p.eq.gain2200;
           self.fx.eq.gains[4] = p.eq.gain8000;

           self.fx.grain_delay.enabled = p.grain_delay.enabled;
           self.fx.grain_delay.time = p.grain_delay.time;
           self.fx.grain_delay.scatter = p.grain_delay.scatter;
           self.fx.grain_delay.density = p.grain_delay.density;
           self.fx.grain_delay.mix = p.grain_delay.mix;

           self.fx.bitcrusher.enabled = p.bitcrusher.enabled;
           self.fx.bitcrusher.bits = p.bitcrusher.bits;
           self.fx.bitcrusher.rate_reduction = p.bitcrusher.rate_reduction;
           self.fx.bitcrusher.mix = p.bitcrusher.mix;

           self.fx.shimmer_verb.enabled = p.shimmer_verb.enabled;
           self.fx.shimmer_verb.shimmer = p.shimmer_verb.shimmer;
           self.fx.shimmer_verb.space = p.shimmer_verb.space;
           self.fx.shimmer_verb.mix = p.shimmer_verb.mix;

           self.fx.distortion.enabled = p.distortion.enabled;
           self.fx.distortion.drive = p.distortion.drive;
           self.fx.distortion.tone = p.distortion.tone;
           self.fx.distortion.mix = p.distortion.mix;

           self.fx.juno_chorus.enabled = p.juno_chorus.enabled;
           self.fx.juno_chorus.mode = p.juno_chorus.mode;
           self.fx.juno_chorus.mix = p.juno_chorus.mix;

           self.fx.ring_mod.enabled = p.ring_mod.enabled;
           self.fx.ring_mod.carrier_hz = p.ring_mod.carrier_hz;
           self.fx.ring_mod.mix = p.ring_mod.mix;

           self.fx.tremolo.enabled = p.tremolo.enabled;
           self.fx.tremolo.rate = p.tremolo.rate;
           self.fx.tremolo.depth = p.tremolo.depth;
           self.fx.tremolo.waveform = p.tremolo.waveform;
           self.fx.tremolo.mix = p.tremolo.mix;

           self.fx.wavefolder.enabled = p.wavefolder.enabled;
           self.fx.wavefolder.drive = p.wavefolder.drive;
           self.fx.wavefolder.folds = p.wavefolder.folds;
           self.fx.wavefolder.mix = p.wavefolder.mix;

           // Slot layout
           self.fx.slot_types = p.fx_slots;
    }
