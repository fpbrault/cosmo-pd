use crate::synthesis::pd::algorithms::{AlgoControlKindV1, algo_definitions_v1};
...
fn algo_definition_for(
    algo: super::Algo,
) -> Option<&'static crate::synthesis::pd::algorithms::AlgoDefinitionV1> {
    algo_definitions_v1()
        .iter()
    ...
    fn virtual_algo_control_mapping_tracks_dynamic_algo_changes() {
        ...
        assert!(
            params
                ...
                .flatten()
                .any(|entry| {
                    entry.id == AlgoControlId::BendCurve
                        && (entry.value - 0.25).abs() < f32::EPSILON
                })
        );
        ...
        assert!(
            params
                ...
                .flatten()
                .any(|entry| {
                    entry.id == AlgoControlId::FoldStages
                        && (entry.value - 0.75).abs() < f32::EPSILON
                })
        );
    }
}
