...
impl LoFiFx {
    ...
    pub fn process(&mut self, sample: f32) -> f32 {
        ...
        let wobble = sinf(self.wobble_phase * TWO_PI) * self.wow_depth.clamp(0.0, 1.0);
        let flutter = sinf(self.flutter_phase * TWO_PI) * self.flutter_depth.clamp(0.0, 1.0);
        ...
        let dropout =
            0.86 + 0.14 * sinf(self.wobble_phase * TWO_PI * 0.37 + self.flutter_phase * TWO_PI);
        let drive_gain = 1.0 + degrade * 12.0;
        let saturated = tanhf(shaped * dropout * drive_gain) / (1.0 + degrade * 1.5);
