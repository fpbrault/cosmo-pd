...
import type { AlgoSlotViewModel } from "../phase-lines/phaseLineTypes";
...
vi.mock("../waveforms/BaseWaveSelector", () => ({
	BaseWaveSelector: ({ title }: { title: string }) => <div>{title}</div>,
}));

vi.mock("../waveforms/SingleCycleDisplay", () => ({
	SynthSingleCycleDisplay: () => <div data-testid="cycle-display" />,
}));

vi.mock("../phase-lines/PerLineParametersCard", () => ({
	default: () => <div data-testid="per-line-params" />,
}));

vi.mock("@/components/controls/parameters/SynthParamSlider", () => ({
	default: ({
		onChange,
		modDestination,
	}: {
