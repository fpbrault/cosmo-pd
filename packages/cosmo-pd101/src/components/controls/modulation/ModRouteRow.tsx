...
const ModRouteRow = memo(function ModRouteRow({
	...
	return (
		...
		>
			{/* Source badge */}
			<span
				className={`shrink-0 rounded border px-1 py-0.5 font-bold font-mono text-[0.52rem] uppercase tracking-[0.15em] ${meta.colorClass} ${meta.bgClass}`}
			>
				{meta.shortLabel}
			</span>
			...
			{showDestination && (
				<>
					...
					<span
						className="min-w-0 flex-1 truncate font-mono text-[0.55rem] text-cz-cream-dim uppercase tracking-widest"
			...
			<Button
				...
				className={`btn btn-sm shrink-0 px-1.5 py-0.5 font-bold font-mono text-5xs uppercase tracking-[0.12em] ${
					route.enabled
			...
			<Button
				...
				className="btn btn-ghost btn-square btn-xs h-4 w-4 shrink-0 text-[0.6rem] text-cz-cream-dim/60 hover:bg-red-500/20 hover:text-red-400"
			>
				✕
			</Button>
		</div>
