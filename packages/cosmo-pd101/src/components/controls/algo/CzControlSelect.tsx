...
interface CzControlSelectProps {
	...
	return (
		<div className="space-y-1.5">
			...
			<div
				...
				data-testid={`cz-control-options-${control.id}`}
			>
				{options.map((option) => {
							...
							{(hoverHandlers) => (
								<Button
									...
									className={[
										...
										useSingleLineLayout
											? "min-w-0 flex-1 overflow-hidden px-0"
											: "h-6 min-w-0 px-0",
									].join(" ")}
								>
									{isPresetControl ? (
...
const CzControlSelect = memo(CzControlSelectInner);

export default CzControlSelect;
