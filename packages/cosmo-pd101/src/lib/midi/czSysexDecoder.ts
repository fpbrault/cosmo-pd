...
export function decodeCzPatch(sysexData: Uint8Array): DecodedPatch | null {
	...
	// Fine detune is stored in the high 6 bits (bits 2..7) as quarter-units.
	// Ignore low 2 bits and decode via right shift so values are stable.
	const detuneFine = Math.max(0, Math.min(60, pdl >> 2));
